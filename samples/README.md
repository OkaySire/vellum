...
| [`RotationAndRewrap`](RotationAndRewrap) | Narrated walkthrough of the 0.2.0 key-lifecycle features: manual DEK rotation, scope-bound (v2) envelopes failing closed cross-scope, and the KEK rewrap tooling (`VellumRewrapService` + `RewrapPayloadAsync`). No external dependencies. |
| [`AspNetCore.Postgres.Vault`](AspNetCore.Postgres.Vault) | Full web-app stack: ASP.NET Core minimal API + Npgsql + HashiCorp Vault Transit, plus background DEK rotation (`Vellum.Rotation`) and the KEK-rotation runbook's rewrap steps as admin endpoints. Ships a `docker-compose.yml` for local Vault + Postgres containers. |
